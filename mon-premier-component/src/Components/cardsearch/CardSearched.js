...
function CardSearched(props) {
  ...
  return props.cardFound !== "" ? (
    <Card className="CommanderCard">
      <Card.Img className="CommanderArt" src={props.card.image_uris.art_crop} />
      ...
      <Card.Subtitle className="Commander Title">{props.card.flavor_text}</Card.Subtitle>
      <Card.Text className="Commander Title">{props.card.mana_cost} - {props.cardFound.card.type_line}</Card.Text>
      <Card.Text className="Commander Oracle Title">{props.card.oracle_text}</Card.Text>
    </Card>
  ) : (
